...
<template>
    ...
    <v-col>
        ...
        <v-sheet v-for="item in quiz.queries" :key="item" elevation="5" rounded class="mb-5 pa-5">
            ...
            <v-text-field @click:append="test(item.query)" append-icon="mdi-open-in-new" :label="'#' + item.index" variant="outlined" v-model="item.query" readonly
            >
                
            </v-text-field>
            <v-text-field v-if="!showAnswer" label="Your Answer (Artist, Title)" v-model="item.yourAnswer" variant="outlined" />
            <v-text-field v-if="showAnswer" label="Your Answer (Artist, Title)" v-model="item.yourAnswer" variant="outlined" readonly />
            <v-text-field v-if="showAnswer" label="Correct Answer (Artist, Title)" variant="outlined" readonly :model-value="item.artist +', '+ item.title" />
        </v-sheet>
